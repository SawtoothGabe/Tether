...
namespace Tether::Platform
{
	...
	void X11Application::PollEvents()
    {
        ...
        if (m_cursorGrabbedWindow)
            m_cursorGrabbedWindow->SetCursorPos(
                m_cursorGrabbedWindow->GetWidth() / 2,
                m_cursorGrabbedWindow->GetHeight() / 2);
    }
    ...
    void X11Application::RedirectEventToWindow(XEvent& event)
    {
        switch (event.type)
        {
            ...
            {
                int revert;
                XGetInputFocus(display, &m_FocusedWindow, &revert);
            }
            break;
        ...
        if (XFindContext(display, event.xany.window, m_UserDataContext,
            reinterpret_cast<XPointer*>(&pWindow)))
        {
            // HACK:
            // On some window managers (I believe this isn't correct behavior)
            // event.xany.window isn't the focused window or the one that
            // the mouse is actually on, so we have to check again to see
            // if the previous XFindContent failed because of this.
            if (event.xany.type == GenericEvent
                && !XFindContext(display, m_FocusedWindow, m_UserDataContext,
                    reinterpret_cast<XPointer*>(&pWindow)) && pWindow)
            {
                pWindow->ProcessEvent(event);
                if (pWindow->m_CursorMode == Window::CursorMode::DISABLED)
                    m_cursorGrabbedWindow = pWindow;
            }
