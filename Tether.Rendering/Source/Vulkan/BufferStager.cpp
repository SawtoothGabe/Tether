...
{
	BufferStager::BufferStager(GraphicsContext& context, const VkBuffer buffer, const size_t bufferSize)
		:
		BufferStager(context, buffer, bufferSize, context.GetQueue(), context.GetCommandPool())
	{}
	...
	BufferStager::~BufferStager()
	{
		...
		m_Dloader.vkFreeCommandBuffers(m_Device, m_CommandPool, 1,
		                               &m_CommandBuffer);
	...
	void BufferStager::UploadDataAsync(const void* data) const
	{
		...
		m_Dloader.vkQueueSubmit(m_Queue, 1, &submitInfo,
		                        m_CompletedFence);
	}
	...
	void BufferStager::RecordCommandBuffer() const
	{
