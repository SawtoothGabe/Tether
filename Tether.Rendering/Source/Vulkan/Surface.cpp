...
#if defined(__linux__) && !defined(TETHER_HEADLESS)
#include <X11/Xlib.h>
#include <X11/Xutil.h>
#include <X11/Xatom.h>
...
Surface::Surface(const GraphicsContext& context)
    :
    ...
    m_Loader(context.GetInstanceLoader())
{
    ...
    if (m_Loader.vkCreateHeadlessSurfaceEXT(m_Instance, &surfaceCreateInfo,
        nullptr, &m_Surface) != VK_SUCCESS)
        throw std::runtime_error("Surface creation failed");
}

#ifndef TETHER_HEADLESS
Surface::Surface(const GraphicsContext& context, Window& window)
	:
	m_Instance(context.GetInstance()),
	m_Loader(context.GetInstanceLoader())
{
